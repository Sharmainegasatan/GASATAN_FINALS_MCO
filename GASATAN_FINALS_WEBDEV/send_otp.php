...
function sendOtpEmail($email, $otp) {
    ...
    try {
        ...
        $mail->SMTPAuth   = true;
        $mail->Username   = 'sharmaineneptunogasatan@gmail.com';
        $mail->Password   = 'khbm grgb jlja mkux';         
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
        $mail->Port       = 587;

        $mail->setFrom('sharmaineneptunogasatan@gmail.com', 'OTP Verification');
        ...
        $mail->Body = "
            ...
            <p><a href='http://localhost/GASATAN_FINALS_WEBDEV/verify.php?email=$email'>Click here to verify</a></p>
        ";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO users (email, otp_code, otp_expire) VALUES (?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sss", $email, $otp, $otp_expiry);

    if ($stmt->execute()) {
        $message = "✅ OTP generated: <strong>$otp</strong> (expires within 5 minutes)<br>";        
